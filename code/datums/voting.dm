...
/datum/vote/proc/submit_vote(var/ckey, var/vote)
	if(mode)
...
/datum/vote/proc/initiate_vote(var/vote_type, var/initiator_key)
	if(!mode)
...
/datum/vote/proc/browse_to(var/client/C)
	if(!C)
		return
...
/datum/vote/proc/update_panel(var/client/C)
	C << output(url_encode(vote_html(C)), "vote.browser:update_vote_div")

/datum/vote/proc/vote_html(var/client/C)
	. = ""
	if(question)
		. += "<h2>Vote: '[question]'</h2>"
	else
